...
**Status:** v1.4 · 2026-07-11 · Phase A of the orchestrated D0–D2+S4 build
...
- **Top nav (constant):** `Now · Features · Delivered · Reports · Issues · Fleet` (+ `Chat`
...
```
┌────────────────────────────────────────────────────────────────────────────────┐
│ factory-dashboard  Now · Features · Delivered · Reports · Issues · Fleet ●SSE ▾  │
...
│ ✖ gate REJECTED 41m [R] — Payments API webhooks ⟨FEAT-9A21⟩ · build-3 · 0.42   │
...
│ Voice session summaries ⟨FEAT-51B0⟩  src:bus    │ │ AGENTS (P1)   as of ● 3s │
│ ●─●─●─●─◉─○─░─░ build · study-tutor  [G]        │ │  forge-orch ● active q:2 │
│ wave 4/6 · 64% · started 12:58 · 38m            │ │  jarvis     ● kv 12s     │
│                                                 │ │  (1h stream — live only, │
│ LPA document intake ⟨FEAT-77AD⟩      src:bus    │ │   no history claimed)    │
│ ●─●─◉─○─○─○─░─░ approval · lpa-poc   [A]        │ │ SERVING (P6) checked ● 8s│
...
- **P2 build board:** each row leads with the **feature title** (§5.8 — id secondary), then
  the mini journey bar (§5.3) + progress + on-track band chip (§5.5). **In-flight progress is
  wave-level only** (wave/wave_total + overall pct + elapsed — `BuildProgressPayload` carries
  no task counters; capability note drift 6); task counts appear at completion from
  BuildComplete. In-flight builds first (by started_at desc), then terminal builds from the
  last 24 h, dimmed. Row click → feature page.
- **P1 roster (condensed):** leads with status dot + queue_depth. Full table on `/fleet`.
  **Dual-source** (capability note drift 3): `fleet.*` subjects AND `$KV.agent-registry.>`
  KV re-puts (core-NATS subscribe — no JS API); KV-sourced agents (jarvis) show `kv ⟨age⟩`
  as their liveness (re-put freshness) with queue/tasks columns unmeasured; register-only
  agents (forge until ask A-10 lands) render "registered ⟨ts⟩ — no heartbeat feed" — a named
  gap, never a fake stale-alarm.
...
```
Study tutor spaced-repetition planning chain ⟨FEAT-3ED2⟩          study-tutor ▸
...
│ ✔ DELIVERED — merged ⟨change⟩ · 2026-07-06 14:13                                  │
...
on-track chip · open issues count), delivered-in-period count (links to `/delivered`
pre-filtered),
...
### 4.4 Delivered — `/delivered` (D2)

**UI naming (Rich's steer, Phase-A review 2026-07-11): the user-facing name is "Delivered"
everywhere** — nav item, `/delivered` route, page heading ("Ledger" conveyed nothing).
`ledger` remains the INTERNAL name only (the design §4.8 table, §5's DDR-DASH-001 semantics,
`backend/ledger.py`) — builders must NOT rename the schema/module to match the UI, nor leak
"ledger" into any rendered copy; the query layer is the seam where the names meet.

```
DELIVERED — operator view                    window: [ 2026-07-01 → 2026-07-08 ▾ ]
...
│ 2026-07-06 │ Study tutor planning chain ⟨FEAT-3ED2⟩│ study-tutor │ Delivered — merged ⟨change⟩   │ ░ pending ░│ ⌗sq  │
│ 2026-07-06 │ Planning wiring fix ⟨FEAT-DD4F⟩       │ study-tutor │ Delivered — merged ⟨change⟩   │ ░ pending ░│ ⌗sq  │
└────────────┴───────────────────────────────────────┴─────────────┴───────────────────────────────┴────────────┴──────┘
(feature column: title leads, id secondary — §5.8; title-less rows fall back to the id with
the gap named in the footer)
events as-of 14:13:34 ● · forge mirror 14:12:58 ●
M-D2 parity: last recorded ⟨date + link to the dated note⟩ | "not yet run"
```

- **Bar labels + evidence (DDR-DASH-001 as amended by dated note 2026-07-11 — Rich's steer:
  the factory's trust mechanism is its gates, not PR process):** the v1 bar is **merged**,
  evidenced by a **merge receipt** — `merge_sha` + repo + branch (rendered `⟨change⟩`, linking
  the merge commit), with `pr_url` as an equally-valid alternate receipt when a flow used a PR
  (a client repo's own branch protection may impose one — per-engagement config, not factory
  doctrine; the link renders whichever receipt exists, PR preferred when both). Labels:
  `Delivered — merged ⟨change⟩` and (later) `Delivered — deployed & live-verified ⟨date⟩`.
  **A COMPLETE build with NEITHER receipt does not clear the bar** — it renders as
  "complete — merge unverified" (a named gap), keeping the delivered claim provable.
- **Launch expectation (capability verification 2026-07-11):** `pr_url` has never been
  populated in any forge DB, so this page is **honestly near-empty at launch** — it fills as
  forge-daemon builds complete with PRs (WS2-V1, w/c 07-14). Pre-daemon deliveries live only
  in guardkit artifacts (by design unscraped); M-D2 names them as feed-gap discrepancies.
...
- The M-D2 parity footer only **displays** the last recorded result — a stored value written by
  the offline parity batch job (its own read-only credential, design §7 F-2g). The `/delivered`
...
│ [—] NO BASELINE Payments API webhooks ⟨FEAT-9A21⟩ — not in the plan         │
│     registry (add to plans.yaml)                                            │
└─────────────────────────────────────────────────────────────────────────────┘
┌─ DELIVERED THIS WEEK ────────────┐ ┌─ ISSUES ENCOUNTERED (window) ──────────┐
│ Study tutor planning chain       │ │ opened 2 · closed 1 · oldest open 2d   │
│  ⟨FEAT-3ED2⟩ merged ⟨change⟩ ⌗sq │ │ ✖ gate REJECTED Payments API webhooks  │
│ Planning wiring fix              │ │   ⟨FEAT-9A21⟩ · build-3 [R]            │
│  ⟨FEAT-DD4F⟩ merged ⟨change⟩ ⌗sq │ │                                        │
└──────────────────────────────────┘ └────────────────────────────────────────┘
┌─ WHERE WE ARE (in flight) ──────────────────────────────────────────────────┐
│ LPA document intake ⟨FEAT-77AD⟩ ●─●─◉─○─○─○─░─░ approval · waiting 22m [A]  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
- **The client-safe export rendering** mechanizes James's outbound firewall: a separate
  template rendering ONLY DF-008-permitted fields (feature id/title/project/bar/date/change
  link — merge commit or PR, the same client-repo link class +
...
| `feat_inflight` | synthetic ⟨FEAT-51B0⟩ "Voice session summaries": build 64%, wave 4/6, `source='bus'`, on-track [G] — NO task counters mid-build (BuildProgress reality, capability note drift 6) | live P2 row; journey bar mid-build; §5.8 title-led row; wave-only progress |
| `feat_amber` | synthetic ⟨FEAT-9A21⟩ "Payments API webhooks": 2nd attempt, 1 gate REJECTED, approval waiting 2h14m — plus one title-LESS feature for the §5.8 id-fallback | needs-you strip; [A]/[R] bands; issues register; title-less fallback + named gap |
...
### 5.8 Feature naming — titles lead, IDs follow (Rich's steer, Phase-A review 2026-07-11)
`FEAT-3ED2` conveys nothing to a human reader. On every **delivery, report, and attention
surface** (Delivered page, Reports, the needs-you strip, the build board, the feature-page
header), a feature renders its projected `title` as the primary label with the FEAT-id as
muted secondary text `⟨FEAT-3ED2⟩` — the id remains the link target and the citation token
the chat uses. **Dense operator event tables** (P3 gate events, watermark detail) may stay
id-first for scanability (id links; title in the `title` attribute). **Honest fallback:** a
feature whose projection has no title renders the id alone and the gap is named once in the
panel footer/coverage ("N features have no projected title — title feed pending"), never an
invented name. Feed reality: the A-8 reduced client event carries `title` explicitly (wire
§4); whether any live operator-side payload does is verified and recorded by S2's P2
liveness note — if none does, the id-fallback stands and a title ask is filed toward B7
(asks-only, owners decide).
...
> `docs/architecture/wire-consumer-requirements-2026-07-08.md` (§4 IN-1/IN-3 + the 2026-07-11
> addendum) · **`docs/notes/capability-verification-2026-07-11.md` (the verified feed reality
> — its §2 dispositions are BINDING; where the design and the note disagree, the note wins,
> per the design's own 2026-07-11 dated addendum).**
...
(unfiltered build board, §2), `/features/{id}`, `/projects/{project}`, `/delivered` (UI name
"Delivered" — §4.4 naming map), `/reports`
...
**Build:** projector connection A (dev creds via env — the interim borrow is the `james`
credential per capability note drift 10; env-var named for the IN-3 `dashboard_ro` swap;
the projector MUST NOT publish or touch the JS API on it — discipline is the enforcement
until IN-3); core-NATS subscriptions `pipeline.> agents.> fleet.>` **plus
`$KV.agent-registry.>`** (KV re-puts ride that subject space — plain subscribe, no JS API,
no consumer; the P1 roster is dual-source per capability note drift 3, with KV entries
honestly-dark until each agent's next re-put); projections P1–P6 (one module per matrix
row) — P3 derives GATED from `StageComplete.gate_mode` + `stage_log` (NO stage-gated
producer exists; if one ever fires its payload uses `stage`/lowercase gate_mode — drift 5);
P4 keys approvals on `request_id` (no `slot_id` exists on the wire — drift 7; decision enum
approve|reject|defer|override); envelope `correlation_id` is nullable — guard it;
`consumer_watermarks` + the 10 s projector heartbeat row (ux §5.6); `forge_mirror.py` (fence 4;
mirror interval 30 s; **the mirror path is explicit config — on this host
`/home/richardwoollcott/forge-prod-state/.forge/forge.db`, the forge-prod bind-mount; NEVER
the `~/.forge` default, which is a stale dev DB written by a `langgraph dev` process —
capability note drift 1**); `health_polls.py` (fence 5); SSE endpoint per design §6 (`id:` =
...
counts, field presence vs the design §2 matrix's ✅/🟡 rows, **including whether any live
payload carries a human feature title (§5.8's feed reality — if none does, record the title
ask toward B7 in the note; asks-only, owners decide)**. Do not edit the design doc; the
note carries the evidence. Tests: ephemeral nats-server (docker) + fixture event streams →
P1–P6 project correctly; the roster dual-source (a `$KV.agent-registry.>` put projects a
KV-sourced agent; a register-only agent renders "no heartbeat feed"); in-flight P2 rows
render wave-only progress; watermarks advance; SSE notifies + replay path; panels render
live/lagging/true-empty/feed-pending/unmeasured per ux §5; mirror opens `mode=ro` (write
attempt fails) and points at the configured path (not the default); the §5.6 F-5 layering tests: heartbeat-stale ⇒ page LAGGING + all stalled
...
**Build:** `ledger.py` — bootstrap from forge `builds` (`status='COMPLETE'` **AND a merge
receipt present — `merge_sha` or `pr_url`, per the amended DDR-DASH-001** → bar `merged_pr`;
COMPLETE with neither receipt → the "complete — merge unverified" named gap, never a
delivered row; join `stage_log`), build-complete consumer appends, idempotent upserts keyed
`(feature_id, bar)`; the ledger schema carries `merge_sha` beside `pr_url` (both nullable);
period query with the F-10b two-list semantics; the `/delivered` page per ux
§4.4 (bar label VERBATIM: `Delivered — merged ⟨change⟩`, evidence = merge receipt per the
amended DDR-DASH-001 — bar clears on `merge_sha` OR `pr_url`, neither ⇒ "complete — merge
unverified" named gap; upgrades list present and labelled).
...
mypy green on your own re-run; the Delivered page renders the §4.4 wireframe against fixtures
with "ledger" in no rendered copy (§4.4 naming map).
...
5. The Delivered page (`/delivered`): bar label verbatim; "ledger" appears in no rendered
   copy (§4.4 naming map); two-list delivered/upgrades layout with the upgrades slot
...
15. Page bodies beyond home/feature/delivered carry their pinned content: `/fleet` shows the §5.6
...
18. `style.css` honours the §7.4 design-portability contract: the stable class-naming set
    ...
    inspection.
19. Feature references are title-led with the id secondary (§5.8) on all delivery/report/
    attention surfaces (Delivered, Reports, needs-you, build board, feature-page header);
    dense event tables may go id-first; the title-less fixture renders the id-fallback with
    the gap named in the footer — never an invented name.
...
**PR-tie removed (Rich's steer, Phase-A review 2026-07-11, fourth pass — v1.4):** Rich
challenged the "merged + PR" bar (PRs = zero-trust process; the factory's trust is its
gates). A code trace confirmed the factory never wired PRs (no programmatic creation, no
auto-merge, `pr_url` hardcoded None at both emit sites, review = gate machinery + human at
the merge boundary). DDR-DASH-001 amended by dated note (design doc): the bar is **merged,
evidenced by a merge receipt** — `merge_sha` primary, `pr_url` alternate (client-repo branch
protection is per-engagement config, not doctrine); neither ⇒ "complete — merge unverified"
named gap, never a delivered row. Labels now `Delivered — merged ⟨change⟩`; ledger schema
gains `merge_sha`; ask **A-11** filed (capture the receipt; un-hardcode pr_url/repo/branch
at the emit sites). The factory-flow question itself (automated direct-merge behind gates)
is WS2/guardkit's — this dashboard is agnostic.

**Capability verification (Rich's ask, Phase-A review 2026-07-11, third pass — v1.3):**
"verify the actual capabilities it wires onto exist" → four read-only repo verifiers + live
GB10 probes (`docs/notes/capability-verification-2026-07-11.md` — BINDING, added to the §9.0
PREFLIGHT list). Headlines: the live forge is the `forge-prod` bind-mount, NOT the `~/.forge`
default (a stale dev DB — mirror path now explicit config); `pr_url` never populated to date
(Delivered honestly near-empty at launch, fills from WS2-V1); P1 roster dual-source (jarvis
is KV-only → `$KV.agent-registry.>` core subscribe, no JS API; forge register-only until
A-10); no stage-gated producer; BuildProgress is wave-only; approvals key on `request_id`;
P5's live emitter has LANDED; no read-only APPMILLA cred (dev-creds = borrowed `james` +
M-D3 discipline); no feature title exists anywhere (§5.8 confirmed; ask A-9 filed). Design +
wire docs carry same-day dated addenda; asks A-9/A-10/IN-3-amendment filed in the wire note.

**Taste-review renames (Rich, Phase-A review 2026-07-11, second pass):** (1) "Ledger" →
**"Delivered"** on every user-facing surface (nav · `/delivered` route · page headings);
`ledger` stays the internal schema/module name — naming map pinned in §4.4, coach-checked
("ledger" in no rendered copy). (2) **Titles lead, IDs follow** (§5.8): `FEAT-3ED2`-style ids
conveyed nothing — delivery/report/attention surfaces render the feature title first with the
id as muted secondary; honest id-fallback + named gap when no title is projected; S2's
liveness note verifies the operator-side title-feed reality (A-8 already guarantees `title`
client-side); acceptance item 19 added.
