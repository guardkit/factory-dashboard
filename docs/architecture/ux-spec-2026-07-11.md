...
**Status:** v1 · 2026-07-11 · Phase A of the orchestrated D0–D2 build
...
**Consumers:** Rich (the Phase-A gate) · the Phase-B Workflow stages S1–S3 (§9 binds them) · the
...
- **Top nav (constant):** `Now · Features · Ledger · Issues · Fleet` (+ `Chat` appears at D4).
  `Features` = the **`/features` route**: the build board expanded to all known features (same
  fragment as P2, unfiltered, plus terminal builds); it needs no separate wireframe.
...
```
┌────────────────────────────────────────────────────────────────────────────────┐
│ factory-dashboard   Now · Features · Ledger · Issues · Fleet     ●SSE  rich ▾  │
...
### 4.7 Fixtures (pinned — tests and coach checks render against these)
...
> 3. Develop and test against an EPHEMERAL local nats-server (docker) + the §4.7 fixtures.
...
without them); `config/tenants.yaml` (operator + one commented example client row);
`base.html` + `client_base.html` per ux §3/§7 incl. the chat-drawer block and banner slot;
panel fragment templates for P1–P7 rendering ALL FIVE ux §5.2 states from the §4.7 fixtures,
**plus the FEED-PENDING Spend/cost slots** (feature + project pages, ledger spend column —
§7.3, not part of P1–P7); routes `/`, `/features` (unfiltered build board, §2),
`/features/{id}`, `/projects/{project}`, `/ledger`, `/issues`, `/fleet`,
`/fragments/{panel}`; `style.css` per ux §8; vendored htmx + SSE ext + `static/freshness.js`
...
loads; every route 200s with fixtures and every top-nav item resolves to a live route; each
...
14. Every top-nav item resolves to a live route (incl. `/features`); no dead links in the
    constant nav.
