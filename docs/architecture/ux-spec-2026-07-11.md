...
- **P2 build board:** each row leads with the **feature title** (§5.8 — id secondary), then
  the mini journey bar (§5.3) + wave/task counters + on-track band chip (§5.5). In-flight
  builds first (by started_at desc), then terminal builds from the last 24 h, dimmed. Row
  click → feature page.
...
| `feat_inflight` | synthetic ⟨FEAT-51B0⟩ "Voice session summaries": build 64%, wave 4/6, tasks 7/11, `source='bus'`, on-track [G] | live P2 row; journey bar mid-build; §5.8 title-led row |
