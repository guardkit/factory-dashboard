...
| A-6 | **Tenant-prefix convention documented as normative** | the `Topics.for_project(project, topic)` prefix (`topics.py:183-198`) + envelope `project` field documented in the topics registry as THE tenant fan-out convention for client-visible delivery events, so `finproxy.pipeline.build-complete.{feat}` is a pinned shape, not an ad-hoc string | convention exists in code; the ask is normative documentation (same genre as WS1-I item 2's `plan-{cid}` ask) |
...
| # | Owner | Ask |
|---|---|---|
| IN-1 | **WS2 B9** (nats-infrastructure) | **Observer-readable pipeline history:** PIPELINE is `retention: work`, 7d/10k (`streams/stream-definitions.json:4-13`) — workqueue semantics make it single-consumer consume-on-ack; an observer cannot replay or even co-consume safely. Requirement (mechanism is B9's choice): limits-retention change, a mirrored observer stream, or a documented ruling that observers get live-subscription-only on PIPELINE subjects (the dashboard's read DB tolerates that — bootstrap comes from forge SQLite either way, design §5) |
...
| IN-3 | **WS5** (accounts, rides a rotation restart) | New APPMILLA service user `dashboard_ro`: **subscribe-only** on `pipeline.> agents.> fleet.> jarvis.> memory.episode.>` + `$JS.>` + `_INBOX.>`; **zero publish grants** — ADR-DASH-007 makes projection-not-participant broker-enforced |
| IN-4 | WS5 + WS2 B9 | **The FinProxy feed pair:** (a) APPMILLA→FINPROXY account export/import of `finproxy.>` (today zero imports/exports exist; FINPROXY has one user, `mark` — `accounts.conf.template:163-175`); (b) a `dashboard_finproxy` subscribe-only user in the FINPROXY account. Until IN-4 + A-8 land, the FinProxy view is honestly dark (arch §4) |
| A-8 | **WS2 B8** (forge deploy stage — the producer half of A-6) | forge emits tenant-prefixed copies of client-visible delivery events (`build-complete`, deploy/live-gate results) for projects with a tenant mapping, via `Topics.for_project()` + envelope `project` — the one producer change that lights the FinProxy ledger at the account boundary rather than in app code |
