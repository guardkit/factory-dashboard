...
| A-8 | **WS2 B8** (forge deploy stage — the producer half of A-6) | **(Re-cut 2026-07-08 — gate BLOCKER: `finproxy.>` is directly readable by the client's own NATS user, so internal payloads must never appear there.)** forge emits a **distinct, DF-008-reduced, client-facing delivery event** on `finproxy.>` (subject per `Topics.for_project()` + envelope `project`) carrying ONLY: feature id, title, bar cleared (`merged_pr` \| `deployed_live_verified`), delivered date, PR URL, and the per-build spend total (computed at build close from the A-4 rollups forge holds). Never "copies" of BuildComplete/verdict payloads — those carry tasks_failed, branch, evidence refs, all DF-008-forbidden client-side. **Emission is best-effort and non-blocking:** a failed or unauthorized tenant-prefixed publish is logged and dropped — it must never fail, delay, or retry-block the pipeline (the consumer must not reshape the producer's failure modes) |
